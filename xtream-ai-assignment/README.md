...
#### Challenges 1 - 3
Please, open / run the Jupyter notebook **jupyter/data_analysis.ipynb**. It contains all the code and explanations for the
challenges 1 - 3.

#### Challenge 4
Please run the Python script **pipeline/main.py**. It contains the code for the challenge 4. You need to specify the path of the
CSV file containing the new data set as an argument, as well as the path of the file where the model will be saved.
Run the following command:

```
cd pipeline && python main.py --data_path <data_path> --model_path <model_path> --classifier <RandomForest/MLP>
```

#### Challenge 5
The API endpoint is available at the following URL: http://localhost:8080/api/v1/candidate:churn.
Please, run the `make start` command to build the container and start the local server. Then, you can send a POST request
...
The response will be a JSON object containing the churn probabilities for each candidate. The format of the response is
the following:

```
[{
    "enrollee_id": <enrollee_id_of_the_candidate>,
    "churn": 0|1
}]
```

For instance, you can use *curl* to send a POST request to the API endpoint:

```
curl -X POST http://localhost:8080/api/v1/candidate:churn \
   -H 'Content-Type: application/json' \
   -d '[{"enrollee_id": 1, "city": "city_75", "city_development_index": 0.767, "gender": "Male", "relevent_experience": null, "enrolled_university": "no_enrollment", "education_level": "Graduate", "major_discipline": "STEM", "experience": ">20", "company_size": null, "company_type": null, "last_new_job": "1", "training_hours": 36}]'
```

Feature tests for the API are available in the **tests** folder. You can run them with the following command:

```
make test
```

These are useful for test automation and CI/CD pipelines.
