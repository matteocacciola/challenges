...
export const calculateMarginsOfGravatar = () => {
  const width = window.root.offsetWidth + window.scrollX;
  const imagesPerRow = Math.floor(width / IMAGE_SIZE);

  const diff = (window.screen.width - imagesPerRow * IMAGE_SIZE) / 2;

  window.root.style.marginLeft = `${diff}px`;
  window.root.style.marginRight = `${diff}px`;
};